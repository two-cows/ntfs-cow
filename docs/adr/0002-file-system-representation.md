...
Workspace copies must share initial file contents, support independent writes, and remain accessible through ordinary Windows filesystem paths. The first feasibility test clones a single file and verifies that its contents are readable through both paths. The proposed design uses whole-file copy-on-write (CoW), with no block-level accounting, and must allow later redirection of block ranges.
...
- **WinFsp's mounted-filesystem design** can use NTFS backing files, but applications must access clones through its filesystem namespace. Directory mounts provide flexibility, but they do not let this design manage individual files in arbitrary existing NTFS directories. Direct access to a live source through its native NTFS path bypasses the CoW callbacks; protecting shared backing requires an additional mechanism. Reject this design for the destination and source-isolation requirements. The documented [kernel-mode option](https://winfsp.dev/doc/WinFsp-Kernel-Mode-File-Systems/) also creates a WinFsp volume. No documented mode that acts as an NTFS minifilter was identified. This conclusion does not rule out a custom design that combines WinFsp with additional redirection and source protection.

ReFS and differencing virtual disks remain comparisons outside the project scope. ReFS requires a different filesystem, and differencing disks share storage at the virtual-disk level.
...
The driver interprets these backing references and manages their lifetime. It uses supported filesystem operations to create files, rename files, and set reparse points. NTFS maintains the master file table (MFT) records and directory indexes.
...
These steps require synchronization per logical file, shared across all handles and processes that open it. Only one operation at a time may perform the first-write copy for `A`. Concurrent writers must wait for the transition to finish and, on success, use the same private file `T`. Reads and writes must coordinate with the transition so that no read observes an incomplete copy and no write reaches shared `S` or is lost through competing mapping changes. The protocol must also cover cached, memory-mapped, and paging I/O, ownership updates, and failed copies. The synchronization mechanism remains open.
...
- Writes to `A` must leave `B`'s contents unchanged and vice versa. The driver must retain backing storage while any logical owner or outstanding operation needs it.
