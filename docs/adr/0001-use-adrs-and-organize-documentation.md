...
The project needs to preserve the reasoning behind significant architectural and project decisions. A small contributor group favors a process with little maintenance or ceremony.
...
- Store records in `docs/adr/NNNN-short-title.md`, with sequential numbers that are never reused.
...
- When a later decision changes an accepted one, create a new ADR, mark the earlier record Superseded, and link both records to each other. Preserve the earlier reasoning; editorial corrections can be made in place.
- Keep learning notes and general educational material out of the repository. Where understanding code requires a specific mechanism, link directly to relevant documentation from the code.
